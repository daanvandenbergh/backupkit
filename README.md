...
```jsonc
{
    ...
    "targets": {
        "web1-www": {
            ...
            "destination": "/srv/backups/web1-www", // this target's archive -> /srv/backups/web1-www/<snapshot>/
...
**`destination` is that target's own directory**, and it means the same thing in both modes: a snapshot target writes `<destination>/<snapshot>/` into it, a mirror writes `<destination>` itself. Backupkit never appends the target name, so give every target a destination of its own - two targets pointed at one directory is a config error, named as such.

Then `backupkit run` performs one pass over every due target; `backupkit start` (or the installed service) does it on schedule.

> **Upgrading from 1.x.** `destination` used to be a *shared parent* that backupkit appended the target name to, so `"destination": "/srv/backups"` for target `web1-www` wrote to `/srv/backups/web1-www/<snapshot>/`. It is now the archive root itself. Nothing on disk has to move - **append the target name to each snapshot target's `destination`** (`/srv/backups` -> `/srv/backups/web1-www`) and the existing archive is exactly where 2.x expects it. `backupkit check` finds a local archive left one level down and prints the destination to set. Push targets also need `sudo backupkit jail install` rerun on the archive server: the 2.x client sends a path an older jail rejects, so every push fails until it is updated. Mirror targets are unaffected.
