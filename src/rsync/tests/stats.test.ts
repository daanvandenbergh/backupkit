...
describe("parseStats2", () => {
    ...
    // rsync relays messages GENERATED BY THE REMOTE onto the local client's
    // stdout, ahead of the client's own stats block. Taking the FIRST match
    // therefore let a compromised source pick every number backupkit acts on:
    // `totalTransferredSize: 0` walks the disk guard past a full filesystem,
    // and an inflated `totalFiles` silences the content-collapse tripwire -
    // the only content signal protecting the archive (invariant 27).
    it("takes the LAST match, so a fake block relayed from the remote cannot win", () => {
        const planted = [
            "Number of files: 999,999,999 (reg: 999,999,999)",
            "Number of regular files transferred: 0",
            "Total transferred file size: 0 bytes",
        ].join("\n");
        expect(parseStats2(`${planted}\n${FULL_STATS}`)).toEqual({
            filesTransferred: 812,
            totalFiles: 120433,
            totalTransferredSize: 10485760,
        });
    ...
    it.each([
        ["empty output", ""],
        // Number.isFinite alone has no length bound: 400 digits parse to
        // Infinity, and an Infinite delta or file count makes every threshold
        // comparison downstream meaningless.
        [
            "an unbounded digit run that would parse to Infinity",
            `Number of files: ${"9".repeat(400)}\nNumber of regular files transferred: 2\nTotal transferred file size: 9 bytes`,
        ],
        [
            "a value above the safe-integer range",
            "Number of files: 3\nNumber of regular files transferred: 2\nTotal transferred file size: 90071992547409910 bytes",
        ],
