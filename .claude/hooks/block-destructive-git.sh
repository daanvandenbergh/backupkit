...
#
# It scans the WHOLE command string. That is NOT because permission rules miss compound commands -
# Claude Code splits on `&&`, `||`, `;`, `|`, `&` and newlines and matches each subcommand, so a
# `Bash(git stash*)` deny rule does catch `cd apps/devsuite && git stash`. The full-string scan buys the
# forms a glob cannot express: a doubled space between `git` and the verb, and a command assembled
# in a variable and `eval`ed. It costs false positives the other way - a guarded string quoted
# INSIDE a harmless command is refused too, which has cost this repo several round-trips. That
# trade is deliberate: a refused `echo` costs a retry; the thing on the other side does not.
#
# WHY THIS IS A HOOK AND NOT JUST `permissions.deny` - the obvious question, answered once:
#   - deny beats allow and CANNOT carry exceptions. `deny: Bash(git stash*)` would also kill
#     `stash list/show/pop/apply` - the recovery path, the one thing that must stay open.
#   - a deny rule carries no explanation. Half of this file is the message: what to do instead,
#     and the line telling the agent not to go looking for a way around it.
# What deny IS better at is reliability: it is pure config and cannot fail open, whereas a hook
# that times out or emits malformed JSON lets the tool call PROCEED. So the two belong together -
# deny as the fail-closed backstop for rules needing no exception, this hook for the ones that do.
#
...
# `git` plus any run of GLOBAL options that may legally precede a subcommand. Without this every
# pattern below assumed the subcommand sits immediately after `git`, so `git -C /path <verb>`
# walked straight through the lot of them - a one-flag bypass of the entire file.
G='git([[:space:]]+(-[cC][[:space:]]*[^[:space:];&|]+|--git-dir=[^[:space:];&|]+|--work-tree=[^[:space:];&|]+|--namespace=[^[:space:];&|]+|--exec-path=[^[:space:];&|]+|--no-pager|--paginate|-P|--bare|--literal-pathspecs))*[[:space:]]+'
...
scrubbed=$(printf '%s' "$command" | sed -E "s/${G}stash[[:space:]]+(list|show|pop|apply)/GIT_STASH_ALLOWED/g")

refusal=""
case "$scrubbed" in
    *git*stash*)
        if printf '%s' "$scrubbed" | grep -qE "${G}stash"; then
...
if [ -z "$refusal" ]; then
    if printf '%s' "$scrubbed" | grep -qE "${G}restore([[:space:]]|\$)"; then
    ...
    elif printf '%s' "$scrubbed" | grep -qE "${G}checkout[[:space:]]+[^;&|]*--([[:space:]]|\$)"; then
    ...
    elif printf '%s' "$scrubbed" | grep -qE "${G}reset[[:space:]]+[^;&|]*--hard"; then
    ...
    elif printf '%s' "$scrubbed" | grep -qE "${G}clean[[:space:]]+[^;&|]*-[a-zA-Z]*f"; then
    ...
    elif printf '%s' "$scrubbed" | grep -qE "${G}add[[:space:]]+([^;&|]*[[:space:]])?(-A|--all|-u|--update)([[:space:]]|\$)" \
        || printf '%s' "$scrubbed" | grep -qE "${G}add[[:space:]]+[^;&|]*(\.|\*)([[:space:]]|\$|/)"; then
    ...
    elif printf '%s' "$scrubbed" | grep -qE "${G}commit[[:space:]]+([^;&|]*[[:space:]])?-a(m)?([[:space:]]|\$)" \
        || printf '%s' "$scrubbed" | grep -qE "${G}commit[[:space:]]+([^;&|]*[[:space:]])?--all([[:space:]]|\$)"; then
